...
#define NDEF_DEMO_WRITE_EMBEDDED_PROTO    0U   /*!< NDEF menu read EmbeddedProto */
#define NDEF_DEMO_READ_EMBEDDED_PROTO    1U   /*!< NDEF menu write EmbeddedProto */
#define NDEF_DEMO_WRITE_URL        2U   /*!< NDEF menu write 2 records    */
#define NDEF_DEMO_FORMAT_TAG       3U   /*!< NDEF menu format tag         */

#if NDEF_FEATURE_ALL
#define NDEF_DEMO_MAX_FEATURES      4U//4U   /*!< Number of menu items         */
#else
...
static const uint8_t *ndefDemoFeatureDescription[NDEF_DEMO_MAX_FEATURES] =
{
	...
	(uint8_t *)"1. Present a tag to write an EmbeddedProto message",
	(uint8_t *)"2. Present a tag to read an EmbeddedProto message",
	(uint8_t *)"3. Present a tag to write a URI record",
...
#if NDEF_FEATURE_ALL
static uint8_t ndefURI[]          = "embeddedproto.com";
...
static uint8_t              ndefDemoFeature     = NDEF_DEMO_WRITE_EMBEDDED_PROTO;
static uint8_t              ndefDemoPrevFeature = 0xFF;
static bool                 verbose             = false;

static uint8_t              rawMessageBuf[DEMO_RAW_MESSAGE_BUF_LEN];
static uint8_t              EmbeddedProtoMessageBuf[50];
...
static void checkUserButton(void)
{
    ...
    if( platformGpioIsLow(PLATFORM_USER_BUTTON_PORT, PLATFORM_USER_BUTTON_PIN))
    {
        ...
        if( ndefDemoFeature != NDEF_DEMO_WRITE_EMBEDDED_PROTO )
        {
...
static void ndefShowDemoUsage()
{
    ...
    platformLog("In Modes 2, 3 or 4, the demo returns to write EmbeddedProto message mode (menu 1) if no tag detected after %d seconds\r\n\n", NDEF_WRITE_FORMAT_TIMEOUT/1000);
#endif /* NDEF_FEATURE_ALL */
}
...
void demoCycle( void )
{
    ...
    if( (ndefDemoFeature != NDEF_DEMO_WRITE_EMBEDDED_PROTO) && (platformTimerIsExpired(timer)) )
    {
        platformLog("Timer expired, back to Write Embedded Proto message mode...\r\n");
        ndefDemoFeature = NDEF_DEMO_WRITE_EMBEDDED_PROTO;
    }
...
static void demoNdef(rfalNfcDevice *pNfcDevice)
{
    ...
    ndefConstBuffer EmbeddedProtoMessage;
    ...
    case NDEF_DEMO_WRITE_EMBEDDED_PROTO:
    	...
    	err |= ndefMessageAppend(&message, &record1); /* Append EmbeddedProto record to message */
		...
		/*
		 * EmbeddedProto read message(s) from tag
		 */
        case NDEF_DEMO_READ_EMBEDDED_PROTO:
        	ndefDemoFeature = NDEF_DEMO_WRITE_EMBEDDED_PROTO; /* returns to Write EmbeddedProto mode after write */
        	...
        	if (verbose)
        	{
        		platformLog("\r\n\r\nRaw EmbeddedProto Message:\r\n");
        		for(uint32_t i=0; i<message.record->bufPayload.length; i++){
        			platformLog("%u ",message.record->bufPayload.buffer[i]);
        		}
        		platformLog("\r\n\r\n");
        	}
		...
		/*
		 * Demonstrate how to encode a URI record, how to encode the message to a raw buffer and then how to write the raw buffer
		 */
		case NDEF_DEMO_WRITE_URL:
			ndefDemoFeature = NDEF_DEMO_WRITE_EMBEDDED_PROTO;  /* returns to Write EmbbededProto message mode after write */
			err  = ndefMessageInit(&message);  /* Initialize message structure */
