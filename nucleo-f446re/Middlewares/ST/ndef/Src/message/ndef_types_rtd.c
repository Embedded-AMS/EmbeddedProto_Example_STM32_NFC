...
static const uint8_t ndefRtdTypeEpr[]            = "EmbeddedProto"; /*!< External Type (EmbeddedProto Record)  */
...
const ndefConstBuffer8 bufRtdTypeEpr             = { ndefRtdTypeEpr,        sizeof(ndefRtdTypeEpr) - 1U };        /*!< EmbeddedProto External Type Record buffer       */
...
/*
 * NFC Forum External Type (EmbeddedProto Record)
 */


/*****************************************************************************/
ReturnCode ndefRtdEpr(ndefType* epr, const ndefConstBuffer* bufPayload)
{
    ndefTypeRtdEpr* rtdEpr;
    ...
    epr->id               = NDEF_TYPE_RTD_EPR;
    epr->getPayloadLength = NULL;
    epr->getPayloadItem   = NULL;
    rtdEpr                = &epr->data.epr;

    rtdEpr->bufType.buffer    = bufRtdTypeEpr.buffer;
    rtdEpr->bufType.length    = bufRtdTypeEpr.length;
    rtdEpr->bufPayload.buffer = bufPayload->buffer;
    rtdEpr->bufPayload.length = bufPayload->length;
...
/*****************************************************************************/
ReturnCode ndefGetRtdEpr(const ndefType* epr, ndefConstBuffer* bufEprString)
{
    const ndefTypeRtdEpr* rtdEpr;

    if ( (epr          == NULL) || (epr->id != NDEF_TYPE_RTD_EPR) ||
         (bufEprString == NULL) )
    {
        return ERR_PARAM;
    }
    ...
    bufEprString->buffer = rtdEpr->bufPayload.buffer;
    bufEprString->length = rtdEpr->bufPayload.length;
...
/*****************************************************************************/
ReturnCode ndefRecordToRtdEpr(const ndefRecord* record, ndefType* epr)
{
    if ( (record == NULL) || (epr == NULL) )
    {
        return ERR_PARAM;
    }

    if ( ! ndefRecordTypeMatch(record, NDEF_TNF_RTD_EXTERNAL_TYPE, &bufRtdTypeEpr)) /* "EmbeddedProto" */
    {
        return ERR_PROTO;
    }
    ...
    return ndefRtdEpr(epr, &record->bufPayload);
}


/*****************************************************************************/
ReturnCode ndefRtdEprToRecord(const ndefType* epr, ndefRecord* record)
{
    const ndefTypeRtdEpr* rtdEpr;
    ...
    /* "EmbeddedProto" */
    (void)ndefRecordSetType(record, NDEF_TNF_RTD_EXTERNAL_TYPE, &bufRtdTypeEpr);

    (void)ndefRecordSetPayload(record, &rtdEpr->bufPayload);
